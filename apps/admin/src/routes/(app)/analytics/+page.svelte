<script lang="ts">
	...
	import { getAnalyticsData } from "$lib/model/analytics/queries.remote";

	const initialRange = Number(page.url.searchParams.get("range") ?? "30");
	const initialLinkId = (page.url.searchParams.get("link") ?? "").trim() || null;

	let selectedRange = $state(initialRange);
	let selectedLinkId = $state<string | null>(initialLinkId);

	const analytics = $derived(
		await getAnalyticsData({ range: selectedRange, linkId: selectedLinkId }),
	);
	...
	const series = $derived.by(() => buildSeries(analytics.clicksByDay));
	...
	const deviceEntries = $derived.by(() => (analytics.devices ?? []) as DeviceEntry[]);
	const browserEntries = $derived.by(() => (analytics.browsers ?? []) as BrowserEntry[]);
	const referrerEntries = $derived.by(() => (analytics.referrers ?? []) as ReferrerEntry[]);
	const countryEntries = $derived.by(() => (analytics.countries ?? []) as CountryEntry[]);
	...
	const topDayLabel = $derived.by(() =>
		formatRangeDate(`${analytics.topDay?.day ?? ""}T00:00:00Z`),
	);
	const topDayClicks = $derived.by(() => analytics.topDay?.clicks ?? 0);
	const topLocationLabel = $derived.by(() => {
		if (!analytics.topCountries?.length) return "No location data";
	...
	const rangeTotal = $derived.by(() => analytics.rangeTotalClicks ?? 0);
	...
	const syncUrl = () => {
		...
		if (selectedLinkId) params.set("link", selectedLinkId);
		goto(`?${params.toString()}`, { replaceState: true });
	};

	const handleRangeChange = (event: Event) => {
		const target = event.currentTarget as HTMLSelectElement;
		selectedRange = Number(target.value);
		syncUrl();
	};

	const handleLinkChange = (event: Event) => {
		const target = event.currentTarget as HTMLSelectElement;
		selectedLinkId = target.value || null;
		syncUrl();
	};
</script>
...
<svelte:boundary>
		...
		<div class="grid gap-6 lg:grid-cols-2">
			<div class="brutal-card p-6">
				...
				<p class="mt-2 text-xs text-[var(--muted)]">
					{formatRangeDate(analytics.rangeStart)} — {formatRangeDate(analytics.rangeEnd)}
				</p>
			</div>
									...
									{#each locationRows.slice(0, 10) as row, index (getRowKey(row, index))}
										<tr>
											...
											<td class="text-right">
												{rangeTotal === 0
													? "0%"
													: `${((row.clicks / rangeTotal) * 100).toFixed(1)}%`}
											</td>
										</tr>
									{/each}
								{/if}
	...
	{#snippet pending()}
		<div class="flex items-center justify-center py-12">
			<p class="text-sm text-[var(--muted)]">Loading analytics...</p>
		</div>
	{/snippet}
