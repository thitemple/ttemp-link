...
{#if open}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-10 backdrop-blur-sm">
		<button
		...
		>
			...
			<div class="mt-6 rounded-2xl border-2 border-black bg-sky-50 px-6 py-5 shadow-[4px_4px_0px_#000]">
				<a
				...
				{#each shareTargets as target (target.id)}
					{#if target.kind === 'link'}
						<a
							...
							rel="noreferrer"
						>
							<div class="grid h-12 w-12 place-items-center rounded-full border-2 border-black bg-white">
